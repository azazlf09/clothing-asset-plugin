...
## 八·补6 · M18 教程更新至 M17 + 重打分发包 v0.2.0（2026-07-30）✅

- **教程**：`使用教程.md/.txt` 补齐 M11~M17 新功能——三档→**四档**（增「反推全图」）、**批量反推**（批量选图/一键反推/并发数/一键复制清除）、资产库**服装/全图视图切换**、**标签增删**、**Eagle 式标签筛选**、复制/清除提示词；默认中文说明。
- **分发包**：删旧 `服装资产库插件-v0.1.0.zip`（M8 老版本，缺 M11~M17），新打 `服装资产库插件-v0.2.0.zip`（59104 B，23 文件）。含 M11~M17 全部功能，`providers/` 只有 cli.js/remote.js（**无 generate.js**，生图已回退），config.json apiKey 均空。
- **打包踩坑修正**：`.NET ZipFile` 的 `entryNameEncoding=UTF-8` 只写 UTF-8 字节但**不设 0x800 标志位**，Windows 资源管理器仍可能按 GBK 解释致乱码。改用 **Python `zipfile`**（对含非 ASCII 名的条目自动设 0x800），验证「所有中文名条目均带 UTF-8 标志=True」，双保险。
- 已提交并推 GitHub：`9034121`，本地=远程 main，远程仅存 v0.2.0（v0.1.0 已移除）。
